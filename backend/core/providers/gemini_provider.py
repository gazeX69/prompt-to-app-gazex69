...
class GeminiProvider(BaseProvider):
    ...
    @property
    def client(self):
        api_key = self.custom_api_key or os.getenv("GEMINI_API_KEY")
        base_url = self.custom_base_url or "https://generativelanguage.googleapis.com/v1beta/openai/"
    ...
    def get_default_model(self) -> str:
        return self.custom_default_model or "gemini-1.5-flash"
    ...
    def complete(
        ...
        if not self.is_available():
            raise ValueError("Gemini Provider is not configured (missing GEMINI_API_KEY)")
